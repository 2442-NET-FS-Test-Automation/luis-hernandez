...
USE LibraryDB;
GO
-- GO is MS SQL Server specific - it is a batching statement. It's telling
...
(
    ...
    BirthYear INT NULL,
    -- NULL here signifies that we INTEND for this to maybe be null
...
GO
-- including my GO batch statement for MS SQL Server
...
(
    ...
    -- Fun fact, Identity will not reuse any numbers even if deleted
    FirstName VARCHAR(50) NOT NULL,
    LastName VARCHAR(50) NOT NULL,
...
(
    ...
    -- This will be a foreign key, we'll set the FK constraint below
...
(
    ...
    -- This will remain NULL until the book is actually returned
    ...
    -- DueDate has to be in the future
);
GO
...
VALUES
    ...
    ('Frank', 'Herbert', 1920),
    ('Kent', 'Beck', 1961);
-- Final row in a multi-insert gets the semi-colon
...
INSERT INTO dbo.Book
    (Title, ISBN, PublishedYear, CategoryName, AuthorId,
    TotalCopies, AvailableCopies, Edition)
VALUES
    ('Clean Code', '9780132350885', 2008, 'Software', 1, 3, 3, 1);
...
INSERT INTO dbo.Member
    (FirstName, LastName, Email, JoinedDate)
VALUES
    ('Ada', 'Lovelace', 'ada@example.com', '2025-01-10'),
    -- 1
    ('Grace', 'Hopper', 'grace@example.com', '2025-02-02'),
    -- 2
    ('Alan', 'Turing', 'alan@example.com', '2025-02-20'),
    -- 3
    ('Linus', 'Torvalds', 'linus@example.com', '2025-03-15'),
    -- 4
    ('Margaret', 'Hamilton', 'margaret@example.com', '2025-04-01'),
    -- 5
    ('Dennis', 'Ritchie', 'dennis@example.com', '2025-05-05');  -- 6
GO

INSERT INTO dbo.Book
    (Title, ISBN, PublishedYear, CategoryName, AuthorId, TotalCopies, AvailableCopies, Edition)
VALUES
...
INSERT INTO dbo.Loan
    (BookId, MemberId, DueDate, ReturnDate)
VALUES
    (6, 1, '2026-06-30', NULL);
...
SET Edition = 2
WHERE BookId = 3;
-- If I leave this off, EVERY ROW gets that new value.
...
WHERE Email = 'someone@nowhere.com';
...
SELECT Title, TotalCopies - AvailableCopies AS CopiesOut
FROM dbo.Book;
...
WHERE CategoryName
IN ('Software', 'testing');
-- By default, many SQL RDBMS systems are case-insensitive for comparisons
...
-- If we are trying to do a comparison to assert that something
...
-- By default, SQL order by ASC (if omitted, its ASC)
...
HAVING COUNT(*) > 2
-- I can't use an alias name in HAVING, either a column that exists, or some function
...
INSERT INTO dbo.Category
    (Name, Description)
VALUES
    ('Software', 'Software design and craftsmanship'),
    -- 1
    ('Testing', 'Testing and TDD'),
    -- 2
    ('Process', 'Process and methodology'),
    -- 3
...
VALUES
    ...
    ('Margaret', 'Hamilton', 'margaret@example.com', '2025-04-01'),
    -- 5
    ('Dennis', 'Ritchie', 'dennis@example.com', '2025-05-05');
-- 6

-- Book references CategoryId (1=Software, 2=Testing, 3=Process, 4=Languages); no author column.
INSERT INTO dbo.Book
    (Title, ISBN, PublishedYear, CategoryId, TotalCopies, AvailableCopies, Edition)
VALUES
    ('Clean Code', '9780132350885', 2008, 1, 3, 3, 1),
    -- 1
    ('Clean Architecture', '9780134494166', 2017, 1, 2, 2, 1),
    -- 2
    ('Refactoring', '9780201485677', 1999, 1, 2, 1, 2),
    -- 3
    ('Patterns of Enterprise Application Architecture', '9780321127426', 2002, 1, 1, 1, 1),
    -- 4
    ('Test Driven Development', '9780321146533', 2002, 2, 2, 2, 1),
    -- 5
    ('Extreme Programming Explained', '9780321278654', 2004, 3, 1, 0, 2),
    -- 6
    ('Design Patterns', '9780201633610', 1994, 1, 2, 2, 1),
    -- 7
    ('The Pragmatic Programmer', '9780201616224', 1999, 1, 4, 3, 1),
    -- 8
    ('The Pragmatic Programmer 20th Anniv', '9780135957059', 2019, 1, 2, 2, 2),
    -- 9
    ('Programming Ruby', '9780974514055', 2004, 4, 1, 1, 1);
-- 10
...
VALUES
    ...
    (10, 6),
    (7, 3),
    -- Design Patterns co-authored (Beck)
    (8, 6);
-- The Pragmatic Programmer co-authored (Thomas)

INSERT INTO dbo.Loan
    (BookId, MemberId, LoanDate, DueDate, ReturnDate)
VALUES
    (3, 1, '2026-06-01', '2026-06-15', NULL),
    -- Refactoring, out to Ada
    (6, 2, '2026-05-20', '2026-06-03', NULL),
    -- XP Explained, out to Grace (0 available)
    (8, 3, '2026-05-25', '2026-06-08', '2026-06-04'),
    -- Pragmatic Programmer, returned
    (1, 4, '2026-06-10', '2026-06-24', NULL),
    -- Clean Code, out to Linus
...
FROM dbo.Book
WHERE TotalCopies > ( SELECT AVG(TotalCopies)
FROM dbo.Book );
...
SELECT b.Title,
    (SELECT COUNT(*)
    FROM dbo.Loan l
    WHERE l.BookId = b.BookId) AS TimesLoaned
FROM dbo.Book b
...
VALUES
    (6, 2, DATEADD(DAY, 14, GETDATE())) -- Using Dateadd to generate a due date
...
CREATE OR ALTER PROCEDURE dbo.usp_CheckoutBook
    ...
    @MemberId INT,
    @Days INT = 14
-- how long is the loan for, default 14
AS
BEGIN
    SET XACT_ABORT ON;
    SET NOCOUNT ON;
    -- turns off the "x rows affected message print"
